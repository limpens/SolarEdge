...
{
SemaphoreHandle_t GuiLock = (SemaphoreHandle_t)ptr;

  //
  // wait for lvgl initialization to complete, before starting our lv_task_handler()
  //
  xSemaphoreTake(GuiLock, portMAX_DELAY);
...
esp_err_t LCDInit(SemaphoreHandle_t GuiLock)
{
...
esp_lcd_rgb_panel_config_t panel_config = {
  ...
  {
    //
    // this should wait for the lvgl setup fase to complete, see comment in the lvUpdateTask.
    //

    // 2023/06/04: bind task to app core:
    BaseType_t coreID = 1; //xPortGetCoreID();
    ESP_LOGI(TAG, "Creating task for lv_update on core %d", coreID);
    xTaskCreatePinnedToCore(&lvUpdateTask, "lv_update", 4096, GuiLock, tskIDLE_PRIORITY, nullptr, coreID);
    return ESP_OK;
  }
