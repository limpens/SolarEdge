...
//
// Init: initialize espWifi class and setup network basics
//
...
esp_event_handler_instance_t instance_got_ip;
  ...
  /**
   *  We disable nvs_enable, to prevent storing wifi settings.
   *  When not using the Espressif flash encryption, wifi credentials end up plain-text in flash.
   *  See: https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/storage/nvs_flash.html#nvs-encryption
   *  Embedding credentials and decrypting them in code can prevent credentials to be stored plain-text.
  */
  cfg.nvs_enable = 0; 

  esp_netif_t *myNetIF=esp_netif_create_default_wifi_sta();

  if (hostname) esp_netif_set_hostname(myNetIF, hostname);
...
//
// init_scan_conf: helper function to initialize wifi_scan_config_t object
//
...
//
// espWifi_Up: scan for available networks and associate when a configured network was found
//
...
wifi_scan_config_t scanConf;
  ...
  ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_STA));
  memset(&wifi_config, 0, sizeof(wifi_config_t));// see: https://github.com/espressif/esp-idf/issues/2606
...
//
// espWifi_Task: task to keep wifi connected
//
void espWifi::Task(void *ptr)
{
  ...
  while(true)
  {
    if (e->Up() != ESP_OK) // no credentials or AP found?
    {
      //ESP_LOGI(TAG, "No valid WiFi configuration found, restarting..");
    ...
    // start callback (if supplied) to notify the rest of the code
    if (e->cbState != NULL)
      e->cbState(e);

    // wait for the disconnect bit indefinitely.
    ...
    // delay a moderate period to prevent saturation of dhcp server / wifi access point
    vTaskDelay(2500 / portTICK_PERIOD_MS);
  }
}

//
// espWifi_Run: start separate task (espWifi_Task) to keep track of the network state
//
...
void espWifi::SetStateCallback(esp_wifi_state_callback cb)
{
  cbState = cb;
}

void espWifi::SetAuthCallback(esp_wifi_auth_callback cb)
{
  cbAuth = cb;
}

void espWifi::SetUserPointer(void *ptr)
{
  userptr = ptr;
}

void *espWifi::GetUserPointer(void)
{
  return userptr;
}
